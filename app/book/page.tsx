            ...
            <div className="max-w-4xl mx-auto px-6">
                ...
                <div className="bg-white rounded-2xl p-8 border border-slate-200 mb-8">
                        ...
                        {sessionTypes.map((s) => (
                            <div key={s.title} className={`border-l-4 ${s.color} bg-slate-50 p-5 rounded-r-xl`}>
                                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
                                    ...
                                    <span className="flex items-center gap-1 text-sky-700 font-semibold text-sm"><IndianRupee size={14} /> {s.price}</span>
                                </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4 mb-8">
                        {packages.map((pkg) => (
                            <div key={pkg.name} className="bg-sky-50 border border-sky-200 p-5 rounded-xl text-center">
                                <h3 className="font-bold text-sky-900 mb-2">{pkg.name}</h3>
                                <p className="text-xl font-bold text-sky-700 mb-1">{pkg.price}</p>
                                <p className="text-sky-500 text-xs">{pkg.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="text-center mb-10 mt-12">
                    <p className="text-2xl text-sky-800 font-['Instrument_Serif',serif] italic">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <div className="bg-sky-50 border border-sky-100 p-6 rounded-2xl flex gap-4">
                        <Shield className="text-sky-500 shrink-0" size={24} />
                        <div>
                            <h3 className="font-bold text-sky-900 mb-2">Confidentiality Note</h3>
                            <p className="text-sm text-sky-800">Your privacy matters. All sessions are completely confidential, and you may also choose anonymous counselling if you are not comfortable sharing your identity.</p>
                        </div>
                    </div>
